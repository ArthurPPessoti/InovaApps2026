  ...
  return <div className="data-page adaptive-data-page">
    ...
    <section ref={stepPanelRef} className="analysis-step-panel" tabIndex={-1} aria-labelledby={`analysis-step-${step}-title`}>
      ...
      {step === 2 && canConfigure && config && <>
        ...
        {mode === "guided" ? <section className="analysis-chat-layout"><article className="panel analysis-chat"><div className="chat-history">{messages.map((item, index) => <div key={`${item.role}-${index}`} className={`chat-message chat-message--${item.role}`}><span>{item.role === "assistant" ? <Sparkle size={15}/> : "Você"}</span><p>{item.text}</p></div>)}{busy === "assistant" && <div className="chat-message chat-message--assistant"><span><Sparkle size={15}/></span><p>Analisando sua resposta...</p></div>}</div><section className="guided-weights" aria-labelledby="guided-weights-title"><header><div><span>Sugestão inicial da IA</span><h3 id="guided-weights-title">Qual é a importância de cada coluna?</h3><p>O número é um peso relativo, não uma porcentagem. Clique para adaptar ao seu negócio.</p></div><strong aria-live="polite">{adjustedWeights ? `${adjustedWeights} ajustado(s)` : `${guidedMetrics.length} sugerido(s)`}</strong></header><div className="guided-weight-list">{guidedMetrics.map((metric) => <article className="guided-weight-row" key={metric.id}><div className="guided-weight-name"><strong>{metric.meaning || metric.column}</strong><span>{metric.sheet} · {metric.column}</span></div><div className="weight-pills" role="radiogroup" aria-label={`Importância de ${metric.meaning || metric.column}`}>{importanceOptions.map((option) => <button key={option.value} type="button" role="radio" aria-checked={metric.importance === option.value} className={metric.importance === option.value ? "is-selected" : ""} onClick={() => updateMetric(metric.id, { importance: option.value, numericWeight: option.weight })}><span>{option.label}</span><small>{option.weight}</small></button>)}</div></article>)}</div><button type="button" className="link-button guided-weights-edit" onClick={() => setMode("manual")}><SlidersHorizontal size={14}/> Ver todas as colunas e configurações</button></section><form className="chat-composer" onSubmit={onChatSubmit}><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Explique o objetivo ou uma prioridade que não aparece acima"/><button className="primary-button" aria-label="Enviar" disabled={!message.trim() || Boolean(busy)}><PaperPlaneTilt size={18}/></button></form><div className="consent-row"><span><ShieldCheck size={14}/> A IA recebe cinco linhas de exemplo com dados sintéticos.</span><button type="button" className="link-button" onClick={() => setSampleInfo(true)}><Brain size={14}/> Como funciona</button></div></article></section> : <ConfigEditor config={config} onChange={setConfig}/>}
