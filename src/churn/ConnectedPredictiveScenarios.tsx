...
import { Area, AreaChart, Bar, BarChart, CartesianGrid, Cell, Legend, Line, ResponsiveContainer, Scatter, ScatterChart, Tooltip, XAxis, YAxis, ZAxis } from "recharts";
...
export function ConnectedPredictiveScenarios({ analysis, mode }: { analysis: ChurnAnalysis; mode: "overview" | "details" }) {
  ...
  return <section className={`connected-predictive connected-predictive--${mode}`}>
    ...
    {mode === "details" && <div className="predictive-grid predictive-grid--details"><article className="panel"><div className="panel-heading"><div><span>Cenário de estresse</span><h2>Migração possível entre faixas</h2><p>Compara a classificação atual com um aumento simulado de 25% nas probabilidades.</p></div></div><div className="predictive-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={migration}><CartesianGrid stroke="rgba(255,255,255,.07)" vertical={false}/><XAxis dataKey="band" tick={{ fill: "#8593ae", fontSize: 10 }}/><YAxis allowDecimals={false} tick={{ fill: "#8593ae", fontSize: 10 }}/><Tooltip/><Legend/><Bar dataKey="atual" name="Atual" fill="#0156fc" radius={[6,6,0,0]}/><Bar dataKey="estresse" name="Cenário de estresse" fill="#ff6b78" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer></div></article><article className="panel"><div className="panel-heading"><div><span>Vetores do risco</span><h2>Fatores com maior impacto financeiro</h2><p>Soma do impacto esperado dos clientes em que cada fator apareceu.</p></div></div><div className="predictive-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={drivers} layout="vertical" margin={{ left: 24 }}><CartesianGrid stroke="rgba(255,255,255,.06)" horizontal={false}/><XAxis type="number" tick={{ fill: "#8593ae", fontSize: 10 }}/><YAxis type="category" dataKey="label" width={130} tick={{ fill: "#b9b9b9", fontSize: 9 }}/><Tooltip formatter={(value) => `R$ ${value} mil`}/><Bar dataKey="impact" name="Impacto esperado" fill="#ffba49" radius={[0,6,6,0]}/></BarChart></ResponsiveContainer></div></article><article className="panel panel--wide"><div className="panel-heading"><div><span>Evidência anterior à projeção</span><h2>Clientes com maior deterioração observada</h2><p>Variações reais em uso e SLA nos três meses anteriores à data de referência.</p></div></div><div className="predictive-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={deterioration}><CartesianGrid stroke="rgba(255,255,255,.06)" vertical={false}/><XAxis dataKey="id" tick={{ fill: "#8593ae", fontSize: 10 }}/><YAxis tick={{ fill: "#8593ae", fontSize: 10 }}/><Tooltip/><Legend/><Bar dataKey="uso" name="Variação de uso" fill="#ff6b78"/><Bar dataKey="sla" name="Variação de SLA" fill="#00f3ff"/></BarChart></ResponsiveContainer></div></article></div>}
